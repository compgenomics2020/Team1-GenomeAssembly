...
mkdir ${outputFolder}/spades_output

if ((verbose)); then
        echo "\nStarted genome assembly\n"
fi
ls ${pathToInputFiles} | grep _1.fq.gz | xargs -I gw basename -s _1.fq.gz gw | xargs -I gwa ${spadesPath} --isolate -1 ${pathToInputFiles}/gwa_1.fq.gz -2 ${pathToInputFiles}/gwa_2.fq.gz -t ${threads} -o ${outputFolder}/spades_output/gwa_output
